...
if __name__ == "__main__":
    ...
    for _model_id, _model_value in _models['models'].items():
        if _model_value['name'] == scenario.model_name:
            ...
            models.create_model(model_id)
            break
    else:
        model_id, meta_rule_list = models.create_model()
    policy_id = policies.create_policy(model_id, meta_rule_list)
    pdp_id = pdp.create_pdp(policy_id=policy_id, project_id=project_id)
