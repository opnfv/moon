...
pdp_set: {pdp_set}
    ...
    '''
        [Note 1] Why you throw an exception here, although you can remove the method if not needed
    '''
    ...
    def current_request(self, value):
        '''
            [Note] if we update  the current_Request values, it won't update the variables
            [ _subject, _object, _action] which are used in the current request

            currently the applied a process on them
             - fetching their IDs from cache
             - getting subject assignment from IDs

             if we using this setter to update current_request
             there will be no fetching from cache which may cause an issue while getting subject assignment
        '''
